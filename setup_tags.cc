...
int main(int argc, char** argv) {
    ...
    //unordered map
    std::unordered_map<int, std::vector<double, double, double>> tag_locations = {
        {0, {0.0, 0.0, 0.0}}
    };
    ...
    //define rotation with the algorithm
    int rotation = 90;
    ...
    //converts device id from character array to int
    int id = atoi(argv[1]);
    //creates the id as a videocapture class. Opens device
    VideoCapture device(id);

    //If the device isn't opened, return an error
    if (!device.isOpened()) {
